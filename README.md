...
## Содержание
- [1) Наша команда](#Команда)
- [2) Описание задачи](#Описание)
- [3) Краткий EDA](#EDA)
- [4) Инженерия признаков](#Инженерия)
- [5) Обучение](#Обучение)
- [6) Каггл-скор](#Каггл)
...
![Распределение исходных таргетов](/priznakov_raspred.png)
...
# ВАЖНО И АКТУАЛЬНО ТОЛЬКО НА 26 МАЯ. ФИНАЛЬНЫЙ ПАЙПЛАЙН СОБИРАЛСЯ ЛОКАЛЬНО НА КОМПЬЮТЕРЕ ОДНОГО ИЗ УЧАСТНИКОВ, ПРИ ПЕРЕНОСЕ В КОЛАБ СКОР ЧУТЬ ЗАВЫШАЕТСЯ С 266 ДО 267. МЫ ИСПРАВИМ ЭТО В БЛИЖАЙШЕЕ ВРЕМЯ.

## Пайплайн разделен на два уровня обучения:

### Уровень 1 (Базовые модели): Мы обучаем три разных алгоритма градиентного бустинга (CatBoost, LightGBM, XGBoost).
...
### Уровень 2 (Мета-модель): Мы берем эти OOF-предсказания от трех бустингов и используем их как новые признаки (колонки) для простой линейной модели — Ridge (Гребневая регрессия).
